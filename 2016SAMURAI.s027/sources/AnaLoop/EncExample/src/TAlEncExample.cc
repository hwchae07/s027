...
void book(TAlEncExample* analoop, const char* anafilename)
{
  ...
  if(!called){
    ...
    TArtAnaFile::RegisterEncFactory(Analyser::HIME,    new TAlEncHIMEExampleFactory);
    TArtAnaFile::RegisterEncFactory(Analyser::CATANA,  new TAlEncCATANAExampleFactory);
    TArtAnaFile::RegisterEncFactory(Analyser::SILICONS, new TAlEncSILICONSExampleFactory);
  }
