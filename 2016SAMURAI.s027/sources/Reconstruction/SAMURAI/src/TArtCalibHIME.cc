...
void TArtCalibHIME::ReconstructData()
{
  ...
  for(Int_t i=0;i<GetNumHIMEPla();++i){
    ...
    // if(!(turaw>=0 && turaw<4095)){
    //   std::cout << turaw << std::endl;
    //   turaw = TArtMath::InvalidNum();
      
    // }
    // if(!(tdraw>=0 && tdraw<4095)){
    //   std::cout << tdraw << std::endl;
    //   tdraw = TArtMath::InvalidNum();
    // }
    // if(!(quraw>=0 && quraw<4095)){
    //   std::cout << quraw << std::endl;
    //   quraw = TArtMath::InvalidNum();
    // }
    // if(!(qdraw>=0 && qdraw<4095)){
    //   std::cout << qdraw << std::endl;
    //   qdraw = TArtMath::InvalidNum();
    // }
    ...
    Double_t qucal = quped * para->GetQUCal() + gRandom->Uniform(-0.2,0.2);
    Double_t qdcal = qdped * para->GetQDCal() + gRandom->Uniform(-0.2,0.2);;
    ...
    // std::cout << "qavecal" << qavecal << std::endl;
    // std::cout << "qucal" << qucal << std::endl;
    // std::cout << "qdcal" << qdcal << std::endl;
    ...
    if(para->DirectionIs(para->Horiz) ){
      pos[0] = para->GetDetPos(0) + posoff[0];
      //pos[1] = posslw + para->GetDetPos(1) + posoff[1]; 
      pos[1] = poscal + para->GetDetPos(1) + posoff[1]; 
      //      std::cout << "V: " << "X " << pos[0] << "Y " << pos[1] << std::endl; 
      // } else if(para->DirectionIs(para->Horiz) ){
    } else if(para->DirectionIs(para->Vert) ){
      //pos[0] = posslw + para->GetDetPos(0) + posoff[0]; 
      pos[0] = poscal + para->GetDetPos(0) + posoff[0]; 
      pos[1] = para->GetDetPos(1) + posoff[1];
      // std::cout << "H: " << "X " << pos[0] << "Y " << pos[1] << std::endl; 
    }
