...
public:
  ...
  friend std::ostream & operator << (std::ostream& out, const TArtPPACPara& p) {
    out << "ID: " << p.id << ", ";
    ...
    out << "TYSumGate: " << p.tysum_min << " - " << p.tysum_max << std::endl;
...
private:
  ...
  Double_t  xfactor; //  ns->mm calibration factor depending on delay line. parameter depending on each PPAC. 
  Double_t  yfactor; //  ns->mm calibration factor depending on delay line. parameter depending on each PPAC. 
  Double_t  xoffset; //  geometrical offset of x-plane in x-dir. inside of PPAC. parameter depending on each PPAC. 
  Double_t  yoffset; //  geometrical offset of y-plane in y-dir. inside of PPAC. parameter depending on each PPAC. 
  Double_t  xns_off; // timing offset of x-plane coming from differenct of left hand and right hand side cable length. should be changed for V1190. XML file provided by BigRIPS is for CAMAC readout.
  Double_t  yns_off; // timing offset of y-plane coming from differenct of left hand and right hand side cable length. should be changed for V1190. XML file provided by BigRIPS is for CAMAC readout.
